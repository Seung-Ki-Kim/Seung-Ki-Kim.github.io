...
- 웃음 수치가 0.1 올라갔을 때, 2000개가 넘는 픽셀값들이 어떻게 변해야 자연스럽게 웃는 얼굴을 만들어낼 수 있는가?
...
**ReLU 함수**는 이러한 기울기 소실 현상을 해결하기 위한 함수이다. ReLU 함수는 미분해도 값이 작아지지 않는 함수이다. ReLU 함수는 입력이 0보다 크면 미분값이 항상 1이다. 1은 여러 번 곱해도 값이 변하지 않기 때문에 층이 깊어져도 오차 정보에 손실이 없다.
